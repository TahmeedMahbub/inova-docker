...
trait QueryShorter
{
    ...
    */
    ...
    /*
     |--------------------------------------------------------------------------
     | SEARCH MULTIPLE ARRAY DATA IN SINGLE FIELD
     |--------------------------------------------------------------------------
    */


    public function scopeSearchByArray($query, $filed_name, $data_array)
    {
        $query->when($data_array, function($qr) use($filed_name, $data_array) {
            $qr->where($filed_name, $data_array);
         });
    }
    ...
    /*
     ...
     |--------------------------------------------------------------------------
    */
    public function scopeDateFilter($query, $filed_name, $form, $to)
    {
        $query->when($form, function($qr) use($filed_name, $form) {
           $qr->where($filed_name, '>=', $form);
        })
        ->when($to, function($qr) use($filed_name, $to) {
           $qr->where($filed_name, '<=', $to);
        });
    }
