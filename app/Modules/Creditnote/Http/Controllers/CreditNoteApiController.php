...
class CreditNoteApiController extends Controller
{
    public function getItems($cn_id)
    {
        ...
        $items = DB::table('item')->select('item_name as text', 'id as value')->orderBy('item_name', 'ASC')->get();
        $taxes = DB::table('tax')->select('tax_name as text', 'id as value')->get();
        $accounts = DB::table('account')->select('account_name as text', 'id as value')->orderBy('account_name', 'ASC')->get();
