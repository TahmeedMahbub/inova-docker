...
class ContactReport
{
    ...
    public function all()
    {
        ...
        if($branch_id==1){
            ...
            $this->all = JournalEntry::with('contact')->groupBy('journal_entries.contact_id')->join('users','users.id','=','journal_entries.created_by')->where('users.branch_id',$branch_id)->selectRaw('journal_entries.*')->get();
        }
    ...
    public function findById($branch_id=1, $id=null,$start,$end,$group=null)
    {
        if(is_null($id))
        {
            return [];
        ...
        $all[]=  JournalEntry::join("bank","bank.id","journal_entries.bank_id")
                            ->join('users','users.id','=','journal_entries.created_by')
                             ...
                             ->selectRaw('journal_entries.*, concat("BANK-",LPAD(journal_entries.bank_id,6,0)) as transectionid,bank.particulars as particularsname, bank.total_amount as rate')
        ...
        $all[] =  JournalEntry::join("incomes","incomes.id","journal_entries.income_id")
                            ...
                            ->selectRaw('journal_entries.*,concat("INC-",LPAD(incomes.income_number,6,0)) as transectionid,account.account_name as particularsname, incomes.amount as rate')
       ...
       //invoice
        $all[] =  JournalEntry::join("invoices","invoices.id","journal_entries.invoice_id")
                           ...
                           ->selectRaw('journal_entries.*, concat("INV-",LPAD(invoices.invoice_number,6,0)) as transectionid,
                            (SELECT group_concat(invoice_entries.description)
                            FROM invoice_entries join account on account.id = invoice_entries.account_id
                            WHERE invoice_entries.invoice_id = invoices.id) as particularsname,

                            (SELECT group_concat(invoice_entries.account_id)
                            FROM invoice_entries join account on account.id = invoice_entries.account_id
                            WHERE invoice_entries.invoice_id = invoices.id) as account_id,
                            ...
                            (SELECT group_concat(invoice_entries.rate)
                            FROM invoice_entries join account on account.id = invoice_entries.account_id 
                            WHERE invoice_entries.invoice_id = invoices.id) as rate')
        ...
        $all[] =  JournalEntry::join("invoices","invoices.id","journal_entries.invoice_id")
                            ...
                            ->selectRaw('journal_entries.*,concat("INV-",LPAD(invoices.invoice_number,6,0)) as transectionid,
                              (SELECT group_concat(account.account_name,concat("(",invoice_entries.description,invoice_entries.rate,IFNULL(account.account_name,""),")")) FROM invoice_entries join account on account.id =invoice_entries.account_id WHERE invoice_entries.invoice_id = invoices.id) as particularsname,
        ...
        $all[] =  JournalEntry::join("payment_receives","payment_receives.id","journal_entries.payment_receives_id")
                                ...
                                ->selectRaw('journal_entries.*, concat("PR-",LPAD(payment_receives.pr_number,6,0)) as transectionid ,
                                  payment_receives.reference as particularsname,payment_receives.amount as rate,
        ...
        $all[] =  JournalEntry::join("expense","expense.id","journal_entries.expense_id")
                                ...
                                ->leftjoin('account','account.id','journal_entries.account_name_id')
                                ->selectRaw('journal_entries.*,concat("EXP-",LPAD(expense.expense_number,6,0)) as transectionid,account.account_name as particularsname, expense.amount as rate')
                                ->where('journal_entries.contact_id',$id)
                                ->where('journal_entries.account_name_id','!=',9)
                                ->whereBetween('journal_entries.assign_date',[$start,$end])
                                ->where("journal_entries.debit_credit",1)
        ...
        $all[] =  JournalEntry::join("bill","bill.id","journal_entries.bill_id")
                                ...
                                (SELECT group_concat(bill_entry.description) 
                                FROM bill_entry join account on account.id =bill_entry.account_id 
                                WHERE bill_entry.bill_id = bill.id) as particularsname,

                                (SELECT group_concat(bill_entry.account_id) 
                                FROM bill_entry join account on account.id =bill_entry.account_id 
                                WHERE bill_entry.bill_id = bill.id) as account_id,
                                
                                (SELECT group_concat(bill_entry.rate)
                                FROM bill_entry join account on account.id =bill_entry.account_id 
                                WHERE bill_entry.bill_id = bill.id) as rate')
        ...
        $all[] =  JournalEntry::join("payment_made","payment_made.id","journal_entries.payment_made_id")
                                ...
                                ->selectRaw('journal_entries.*, concat("PM-",LPAD(payment_made.pm_number,6,0)) as transectionid ,payment_made.reference as particularsname, payment_made.amount as rate')
                                ->where('journal_entries.contact_id',$id)
                                ->where("journal_entries.debit_credit",0)
                                ->whereBetween('journal_entries.assign_date',[$start,$end])
    ...
    public function contactList($branch_id = 1, $list=[],$start,$end)
    {
       ...
       try{
           ...
           foreach($list as $key => $contacts)
           {
               ...
               if($contacts['contact']){
                   ...
                   $openning_all                        = $this->openningBalance($branch_id, $contacts->contact['id'], $start);
                   $openning_balance                    = $this->sumDrCR($openning_all);
                   $transactionBalance_all              = $this->transactionBalance($branch_id, $contacts->contact['id'], $start, $end);
    ...
    public function openningBalance($branch_id=1, $id=null,$start)
    {
        if(is_null($id))
        {
            return [];
    ...
    public function transactionBalance($branch_id=1, $id=null,$start,$end)
    {
        if(is_null($id))
        {
            return [];
