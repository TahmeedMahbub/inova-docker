                  ...
                  <div class="user_content">
                      <div class="uk-margin-top">
                          <div class="uk-grid" data-uk-grid-margin>
                              ...
                              <div style="margin-top:25px" class="uk-width-medium-1-6">
                                    ...
                                    <table class="uk-table">
                                        ...
                                        <tbody>
                                          ...
                                          @endphp
                                            ...
                                            @foreach($product_phase_item as $key=>$value)
                                            @php
                                             $sub_qty     = 0;
                                             $sub_rate    = 0;
                                             $sub_amount  = 0;
                                            @endphp
                                            @if(  count($final_project[$key][$value->product_phase_name]) !=0)
                                            <tr>
                                              <td></td>
                                              <td></td>
                                              <td style="color:red" colspan="6">
                                                 {{$final_project[$key]['product_phase_name']}}
                                               </td>

                                            </tr>
                                            @endif
                                             @foreach($final_project[$key][$value->product_phase_name] as $key2=>$data)

                                                <tr>
                                                  <td class="uk-text-center">{{ $key2+1}}</td>
                                                  <td class="uk-text-center">{{ date("d-m-Y", strtotime($final_project[$key]['product_phase_item_date'] ))}}</td>
                                                  <td class="uk-text-center"></td>
                                                  <td class="uk-text-center">RMI-{{  $final_project[$key]['rmi'] }}</td>
                                                  <td class="uk-text-center">{{ $data->item_name }} </td>
                                                  <td class="uk-text-right">{{ $data->total }}</td>
                                                  <td class="uk-text-right">{{number_format($data->item_purchase_rate, 2) }}</td>
                                                  <td class="uk-text-right">{{ number_format($data->item_purchase_rate * $data->total, 2) }}</td>
                                                </tr>
                                                @php
                                                   $sub_qty     += $data->item_purchase_rate;
                                                   $sub_rate    += $data->total;
                                                   $sub_amount  += $data->item_purchase_rate*$data->total;
                                                @endphp
                                              @endforeach
                                              <!-- <tr>
                                                <td class="uk-text-center"></td>
