                    ...
                    <div id="invoice_preview">
                        <div class="md-card-toolbar">
                            <div style="width: 100%" class="md-card-toolbar-actions hidden-print">
                                ...
                                {!! Form::open(['url' => 'report/account/sales/ledger', 'method' => 'POST', 'class' => 'user_edit_form', 'id' => 'invoice-search']) !!}
                                    ...
                                    <div class="uk-modal-dialog">
                                        ...
                                        <div class="uk-width-large-2-2 uk-width-2-2">
                                            ...
                                            <div style="display: none" class="uk-width-large-2-2 uk-width-2-2">
                                                <div class="uk-input-group">
                                                    <label for="branch_id" style="margin-left: 10px;">Sub Category</label>
                        ...
                        <div class="md-card-content invoice_content print_bg">
                            ...
                            <div class="uk-grid" data-uk-grid-margin="">
                                ...
                                <div class="uk-width-small-5-5 uk-text-center ">
                                    ...
                                    <p style="line-height: 0px;" class="heading_b uk-text-black">Invoice Report</p>

                                    @if(isset($branch_id))<p style="margin-top: 10px;margin-bottom: 10px;">@foreach($branch as $branchs) @if($branchs->id==$branch_id) {{$branchs->branch_name}} @endif @endforeach</p>@endif
                                    <p style="line-height: 0px; margin-top: 0px;" class="uk-text-small">From {{ date('d-m-Y',strtotime($start)) }}  To {{ date("d-m-Y",strtotime($end))}}</p>
                                    ...
                                    <table class="uk-table">
                                        <thead>
                                            <tr class="uk-text-upper">
                                                ...
                                                <th style="" class="uk-text-left"style="" >Items</th>
                                                <th class="uk-text-right">Receivable</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            ...
                                            @foreach($invoices as $invoice)
                                                <tr class="uk-text-middle">
                                                    <td class="uk-text-left">{{ date('d-m-Y', strtotime($invoice['invoice_date'])) }}</td>
                                                    <td class="uk-text-left">INV-{{ $invoice['invoice_number'] }}</td>
                                                    <td class="uk-text-left">{{ $invoice['customer']->display_name }}</td>
                                                    <td class="uk-text-left" style="text-align: justify;">{{ $invoice['item_name'] }}</td>
                                                    <td class="uk-text-right">{{ $invoice['total_amount'] }}</td>
                                                </tr>

                                                @php
                                                    $total_amount       = $total_amount + $invoice['total_amount'];
                                                    $due_amount         = $due_amount + $invoice['due_amount'];
                                                    $received_amount    = $received_amount + ($invoice['total_amount'] - $invoice['due_amount']);
                                                @endphp
