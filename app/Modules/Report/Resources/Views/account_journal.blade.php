                            ...
                            @elseif($journalData['journal_type'] == 'payment_made')
                                @if($journalData['journal_type'] == $JournalEntryData->jurnal_type && $journalData['journal_id'] == $JournalEntryData->payment_made_id)
                                    ...
                                    @if($i == 0)
                                        <thead>
                                        <tr class="uk-text-upper">
                                            <th>{{$JournalEntryData->paymentMade->payment_date }} {{ $JournalEntryData->paymentMade->account_id == 3 ? 'CP' : 'BP' }}-{{ \Carbon\Carbon::createFromFormat('Y-m-d', $payment_made->payment_date)->year }}/{{ str_pad(isset($JournalEntryData->paymentMade->pm_number)?$JournalEntryData->paymentMade->pm_number:0, 6, '0', STR_PAD_LEFT) }}</th>
                                            <th class="uk-text-center">DEBIT</th>
                                            <th class="uk-text-center">CREDIT</th>
                                        </tr>
                                        </thead>
