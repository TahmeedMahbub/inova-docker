                    ...
                    <div class="md-card">
                            ...
                            <div class="md-card-toolbar-actions hidden-print">
                                ...
                                <div class="uk-modal" id="coustom_setting_modal">
                                    <div class="uk-modal-dialog">
                                        {!! Form::open(['url' => 'recurring-bill', 'method' => 'get', 'class' => 'user_edit_form', 'id' => 'user_profile']) !!}
                        ...
                        <div class="user_heading">
                            ...
                            <div class="user_heading_content">
                                <h2 class="heading_b"><span class="uk-text-truncate">Recurring Bill List</span></h2>
                            </div>
                        </div>
                        ...
                        <div class="user_content">
                                ...
                                <table class="uk-table" cellspacing="0" width="100%" id="data_table" >
                                    ...
                                    <tbody>
                                        @foreach($recurring_bills as $key=>$recurring_bill)
                                            <tr>
                                                <td>{{ $key+1 }}</td>
                                                <td>{{ date('d-m-y',strtotime($recurring_bill->bill_date)) }}</td>
                                                <td>{{ date('d-m-y',strtotime($recurring_bill->start_date)) }}</td>
                                                <td>RBNo-{{ $recurring_bill->recurring_bill_no }}</td>
                                                <td>{{ $recurring_bill->customer->display_name }}</td>
                                                <td>{{ $recurring_bill->amount }}</td>
                                                @if(Auth::user()->branch_id == 1)
                                                    <td> {{ $recurring_bill->createdBy->branch->branch_name }}  </td>
                                                @endif
                                                <td class="uk-text-center" style="white-space:nowrap !important;">
                                                <a href="{{ route('recurring_bill_show', ['id' => $recurring_bill->id]) }}"><i data-uk-tooltip="{pos:'top'}" title="View" class="material-icons">visibility</i></a>
                                                <a href="{{ route('recurring_bill_edit', ['id' => $recurring_bill->id]) }}"><i data-uk-tooltip="{pos:'top'}" title="Edit" class="material-icons">&#xE254;</i></a>
                                                <a class="delete_btn"><i data-uk-tooltip="{pos:'top'}" title="Delete" class="material-icons">&#xE872;</i></a>
                                                <input type="hidden" class="recurring_bill_id" value="{{ $recurring_bill->id }}">
                            ...
                            <div class="md-fab-wrapper branch-create">
                                <a id="add_branch_button" href="{{ route('recurring_bill_create') }}" class="md-fab md-fab-accent branch-create">
...
@section('scripts')
    <script>
        ...
        $('.delete_btn').click(function () {
            var id = $(this).next('.recurring_bill_id').val();
            swal({
                title: 'Are you sure?',
                ...
                window.location.href = "/recurring-bill/delete/" + id;
            })
        })
    </script>
@endsection
