            ...
            <div class="uk-width-large-2-10 hidden-print uk-visible-large">
                <div class="md-list-outside-wrapper">
                    <ul class="md-list md-list-outside">
                        ...
                        <li class="heading_list">Recent Purchase Bills</li>
            ...
            <div class="uk-width-large-8-10">
                        ...
                        <div class="md-card-content invoice_content print_bg" style="height: 301px;">
                            ...
                            <div class="uk-grid" data-uk-grid-margin>
                                ...
                                <div class="uk-width-5-5" style="font-size: 12px;">
                                    <div class="uk-grid">
                                        <h2 style="text-align: center; width: 90%;" class="">PURCHASE BILL</h2>
                                ...
                                <div class="uk-width-small-1-2 uk-row-first">
                                    <div class="uk-margin-bottom">
                                        ...
                                        <address>
                                            ...
                                            @if(!empty($bill->customer->billing_address))
                                            <p>Billing Address-
                                            {{ $bill->customer->billing_address }}
                                            </p>
                                            @endif

                                            @if(!empty($bill->customer->shipping_address))
                                            <p>Shipping Address-
                            ...
                            <div class="uk-grid uk-margin-large-bottom">
                                <div class="uk-width-1-1">
                                    <table id="table_center" border="1" class="uk-table">
                                        ...
                                        <tr class="uk-table-middle">
                                            <td>{{ $i++ }}</td>
                                            <td>{{ $bill_entry->item->item_name }} @if($bill_entry->description) {!! " " . nl2br($bill_entry->description) !!} @endif</td>
                                        ...
                                        @endif
                                        @if($bill->adjustment != "" && $bill->adjustment != NULL && $bill->adjustment != 0)
                                        <tr class="uk-table-middle">
                                            <td class="no-border-bottom"></td>
                                            <td class="no-border-bottom"></td>
                                            <td class="no-border-bottom"></td>
                                            <td class="uk-text-right no-border-bottom">Adjustment</td>
                                            <td class="uk-text-right no-border-bottom">{{ $bill->adjustment }} {{ $bill['adjustment_type'] == 0 ? '%' : BDT }}</td>
                                        </tr>
                                        @endif
                                        @if ($bill->total_tax > 0)
                                            <tr class="uk-table-middle">
                                                ...
                                                <td class="no-border-bottom"></td>
                                                <td class="no-border-bottom"></td>
                                                <td class="uk-text-right">Vat/Tax</td>
                                                <td class="uk-text-right">{{ $bill['total_tax'] }}</td>
                                            </tr>                                            
                                        @endif
                ...
                <div class="hidden-print">
                    ...
                    <div class="uk-margin-top">
                        <h2 class="heading_b">Credits Used</h2>
                    </div>
                                ...
                                <div class="uk-width-1-1">
                                    <table class="uk-table">
                                        ...
                                        <tr class="uk-text-upper">
                                            ...
                                            <th>Date</th>
                                            <th>Vendor Credit</th>
                                            <th class="uk-text-right">Credits Used</th>
                                            <th class="uk-text-center">Action</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        <?php $i = 1;?>
                                        @foreach($vendor_credit_payments as $vendor_credit_payment)
                                        <tr class="uk-table-middle">
                                            <td>{{ $i++ }}</td>
                                            <td>{{ $vendor_credit_payment->created_at->format('Y-m-d') }}</td>
                                            <td>VC-{{ str_pad($vendor_credit_payment->vendorCredit->vendor_credit_no, 6, '0', STR_PAD_LEFT) }}</td>
                                            <td class="uk-text-right">BDT {{ $vendor_credit_payment->amount }}</td>
                                            <td class="uk-text-center">
                                                {{--<a href="{{ url('/invoice/delete-credit'.'/'.$vendor_credit_payment->id) }}" class="delete_btn"><i data-uk-tooltip="{pos:'top'}" title="Delete" class="md-icon material-icons">&#xE872;</i></a>--}}
                                                <a class="vendor_credit_payment_delete_btn"><i data-uk-tooltip="{pos:'top'}" title="Delete" class="md-icon material-icons">&#xE872;</i></a>
                                                <input type="hidden" class="vendor_credit_payment_id" value="{{ $vendor_credit_payment->id }}">
                                            </td>
                                        </tr>
                                        @endforeach
                                        </tbody>
...
@section('sweet_alert')
    <script>
        ...
        $('.vendor_credit_payment_delete_btn').click(function () {
            var id = $(this).next('.vendor_credit_payment_id').val();
            swal({
                title: 'Are you sure?',
                text: "You won't be able to revert this! If you delete this",
                type: 'warning',
                ...
                window.location.href = "/bill/delete-credit/"+id;
            })
        });
