...
class Payment
{
	...
	public function makePaymentReceive($request, $invoice_id)
	{
		...
		if(Account::find($request->payment_account)->account_type_id == 5 && !empty($request->cheque_number) && !empty($request->issue_date)){
			$newpaymentreceive->cheque_number    = $request['cheque_number'];
			$newpaymentreceive->cheque_issue_date = date("Y-m-d", strtotime($request['issue_date']));
			$newpaymentreceive->cheque_status    = 'unpresented';
		}


		if (!$newpaymentreceive->save()) {
			throw new \Exception("Payment Receive Failed to Add.");
		}
