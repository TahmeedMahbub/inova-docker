...
class AccountChartWebController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        $check_in_journal_entries = JournalEntry::where('account_name_id', $id)->count();
        ...
        }elseif($check_in_journal_entries > 0){

            return redirect()
                ->route('account_chart')
                ->with('alert.status', 'danger')
                ->with('alert.message', 'You have transactions with this account!');
    ...
    public function getAccountChartFixedAsset(Request $request)
    {
        $account_chart = Account::select('id')->where('account_type_id', 6)->first();

        return response()->json($account_chart);
    }
}
