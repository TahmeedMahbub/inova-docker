...
@section('title', 'Edit Receive Materials')
...
@section('content')
    <div class="uk-grid" data-uk-grid-margin data-uk-grid-match id="user_profile">
        <div class="uk-width-large-10-10">
            {{ Form::open(['url' => route('phase_receive_update', ['id' => $manufacture_phase->id]), 'method' => 'POST', 'class' => 'uk-form-stacked', 'id' => 'user_edit_form']) }}
                        ...
                        <div class="md-card">
                            <div class="user_heading">
                                ...
                                <div class="user_heading_content">
                                    <h2 class="heading_b"><span class="uk-text-truncate">Edit Receive Materials</span></h2>
                                </div>
                            </div>
                                ...
                                <div class="uk-margin-top">
                                    ...
                                    <div class="uk-grid uk-margin-large-bottom" data-uk-grid-margin>
                                        <div class="uk-width-medium-1-1">
                                            <table class="uk-table">
                                                <thead>
                                                    <tr>
                                                        <th class="uk-text-nowrap">Receive Materials</th>
                                                        <th class="uk-text-nowrap">CTN</th>
                                                        <th class="uk-text-nowrap">PCS</th>
                                                        <th class="uk-text-nowrap">Receive Date</th>
                                                    ...
                                                    @foreach ($manufacture_phase->manufacturePhaseReceivesFromFactory as $key => $receive)
                                                        <tr data-section-added="{{ $key }}">
                                                            <td style="padding-top: 25px">
                                                                <input type="hidden" id="item_id_{{ $key }}" value="{{ $receive->variation->item_id }}">
                                                                <select name="item_variation[]" id="item_variation_{{ $key }}" onchange="variationChanged(this)" class="select2-single-search-dropdown uk-margin-medium-top">
                                                                    <option value="">Select Receive Material</option>
                                                                    @foreach($raw_materials as $raw_material)
                                                                        <option value="{{ $raw_material->id }}" data-item-id={{ $raw_material->item->id }} {{ $receive->variation_id == $raw_material->id ? 'selected' : '' }}>{{ $raw_material->variation_name }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </td>
                                                            <td>
                                                                <input name="quantity_ctn[]" type="text" id="quantity_ctn_{{ $key }}" class="md-input ctn" oninput="calculateCtnToPcs(this)" value="{{ number_format($receive->variation->carton_size == 0 ? ($receive->variation->item->carton_size == 0 ? 0 :  $receive->quantity / $receive->variation->item->carton_size) : $receive->quantity / $receive->variation->carton_size , 3) }}" placeholder="Enter quantity" />
                                                                @if ($errors->has('quantity_ctn.'.$key))
                                                                    <div class="uk-text-danger uk-text-center">{{ $errors->first('quantity_ctn.*') }}</div>                                                                        
                                                                @endif
                                                            </td>
                                                            <td>
                                                                <input name="quantity_pcs[]" type="text" id="quantity_pcs_{{ $key }}" class="md-input pcs" oninput="calculatePcsToCtn(this)" value="{{ $receive->quantity }}" placeholder="Enter quantity" />
                                                            ...
                                                            <td>
                                                                ...
                                                                name="date[]" value="{{ date('d-m-Y', strtotime($receive->date)) }}" data-uk-datepicker="{format:'DD-MM-YYYY'}">
                                    ...
                                    <div class="uk-grid uk-ma" data-uk-grid-margin>
                                         <div class="uk-width-1-1 uk-float-left">
                                            ...
                                            <a href="{{ route('phase_receive_show', ['id' => $manufacture_phase->id]) }}" class="md-btn md-btn-flat uk-modal-close">Close</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    var items_chosen = [];
    $.each({!! json_encode($manufacture_phase->manufacturePhaseReceivesFromFactory) !!}, function (indexInArray, valueOfElement) {
    ...
    function variationChanged(e){
        ...
        if($(e).val() != ''){
            ...
            UIkit.notify({
                message : 'Please select a receive material',
                status  : 'warning',
                timeout : 2000,
                pos     : 'top-right'
            });
