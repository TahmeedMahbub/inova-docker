...
class PaymentReceivedApiController extends Controller
{
    public function getCustomerList()
    {
        ...
        if($branch_id == 1)
        {
            $contacts = DB::table('contact')->select('display_name as text','first_name', 'last_name', 'id as value')->get();
        } 
        else
        {
            

            $contacts   = Contact::leftjoin('users' , 'users.id', '=', 'contact.created_by')
                                    ->when($branch_id != 1 && $show_all_contact == 0, function ($query) use ($branch_id) {
                                            return $query->where('users.branch_id', '=', $branch_id);
                                        })
                                    ->select('contact.display_name as text','contact.first_name', 'contact.last_name', 'contact.id as value')
                                    ->get();
        }
    ...
    public function getCustomerInvoiceEdit($payment_receive_id)
    {
        ...
        $invoices_due                    = Invoice::where('customer_id', $payment_receive[0]['customer_id'])->where('due_amount', '>', 0)->where('save', null)->whereNotIn('id', $invoice_id_array)->get()->toArray(); //showing all due invoices
    ...
    public function getPaymentReceiveEntry($payment_receive_id)
    {
        ...
        {
            // $contacts =  DB::table('contact')->join('users', 'users.id', 'contact.created_by')->where('users.branch_id', $branch_id)->select('contact.display_name as text','contact.first_name', 'contact.last_name', 'contact.id as value')->get();
            $contacts =  DB::table('contact')->select('contact.display_name as text','contact.first_name', 'contact.last_name', 'contact.id as value')->get();
        }
