...
class PaymentMadeApiController extends Controller
{
   public function getVendorList()
   {
      ...
      if($branch_id == 1)
      {
        $contacts = DB::table('contact')->select('display_name as text','first_name', 'last_name', 'id as value')->where('contact_category_id', 4)->get();
      } 
      else
      {
        $contacts =  DB::table('contact')->join('users', 'users.id', 'contact.created_by')
      ...
      $paid_throughs  = DB::table('account')->select('account_name as text', 'id as value')->whereIn('account_type_id',[5,4])->get();
