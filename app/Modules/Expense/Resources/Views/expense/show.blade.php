            ...
            <div class="uk-width-large-2-10 hidden-print uk-visible-large">
                <div class="md-list-outside-wrapper">
                    <ul class="md-list md-list-outside">
                        ...
                        @foreach($expenses as $expense_val)
                        <li>
                            <a href="{{ route('expense_show', $expense_val->id) }}" class="md-list-content">
                                <span class="md-list-heading uk-text-truncate">{{ $expense_val->customer->display_name }} <span class="uk-text-small uk-text-muted">({{ $expense_val->created_at->format('d M Y') }})</span></span>
                                <span class="uk-text-small uk-text-muted">EXP-{{ str_pad($expense_val->expense_number, 6, '0', STR_PAD_LEFT) }}</span>
                            </a>
                        </li>
                        @endforeach
                        ...
                        <div class="md-card-content invoice_content print_bg" style="height: 301px;">
                            ...
                            @else
                                <div class="uk-grid" data-uk-grid-margin style="text-align: center;">
                                    <h1 id="company_h" style="width: 100%; text-align: center;"><img id="company" style="text-align: center;" class="logo_regular" src="{{ url('uploads/op-logo/logo.png') }}" alt="" height="15" width="71"/><br> {{ $OrganizationProfile->company_name }}</h1>
                                </div>
                                <div class="" style="text-align: center;">
                            ...
                            <div  class="uk-grid" data-uk-grid-margin>
                                <div class="uk-width-5-5" style="font-size: 12px;">
                                    <div class="uk-grid" style="text-align: center;">
                                        <h2 id="payemnt_rec" style=" border: 1px solid black; border-radius: 25px; padding-right: 35px" class="uk-align-center">EXPENSE RECEIPT</h2>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="uk-grid">
                                ...
                                <div id="recieve" class="uk-width-small-1-3 uk-row-first">
                                    ...
                                    <span> #EXP-{{ str_pad($expense->expense_number,6,'0',STR_PAD_LEFT) }} </span>
                                </div>

                                <div class="uk-width-small-5-5 uk-row-first">
                                    <span style="font-size: 16px; font-weight: bold">Paid To : </span>
                                    <span> {{ $expense->customer->display_name }}</span>
                                </div>

                                <div id="recieve" class="uk-width-small-1-3 uk-row-first">
                                    <span style="font-size: 16px; font-weight: bold">Paid Through : </span>
                                    <span> {{ ' '.$expense->accountPaidThrough->account_name }}</span>
                                </div>

                                @if($expense->paid_through_id != 3)
                                    <div class="uk-width-small-2-3 uk-row-first">
                                        <span style="font-size: 16px; font-weight: bold">Expense Account Name : </span>
                                        <span> {{ $expense->account->account_name }} </span>
                                    </div>

                                    <div id="recieve" class="uk-width-small-1-3 uk-row-first">
                                        <span style="font-size: 16px; font-weight: bold">Cheque : </span>
                                        <span> {{ ' '.$expense->bank_info}}</span>
                                    </div>
                                @endif
                            </div>
                                ...
                                <div id="recieve" class="uk-width-small-6-6 uk-row-first">
                                    ...
                                    <table id="info" class="uk-table inv_top_right_table">
                                        ...
                                        <tr class="uk-table-middle">
                                            ...
                                            <td class="uk-text-right">{{ $expense->amount }}</td>
                                        </tr>
                                        <tr class="uk-table-middle">
                                            <td class=" no-border-bottom uk-text-right" style="font-weight: bold;">Total Received</td>
                                            <td class="no-border-bottom uk-text-right" style="font-weight: bold">{{ $expense->amount }}</td>
                                        </tr>
                                    </table>
                                </div>
                            </div>
                            ...
                            <div style="margin-top: 10px" class="uk-grid">
                                <div class="uk-width-small-2-2 uk-row-first">
                                    <span style="font-size: 16px; font-weight: bold">In Words:</span><span>{{' '.ucfirst($numberTransformer->toWords($expense->amount))}} BDT Only</span>
                                </div>
                            </div>
