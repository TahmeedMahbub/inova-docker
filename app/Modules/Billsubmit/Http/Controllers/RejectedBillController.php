...
{    private $branch_id              = 0;
    ...
    public function index(Request $request)
    {
        ...
        {
            if($request->due){
                $bills = BillSubmit::where('due_amount','!=',0)->where('bill_submit.status', '=', 1)->get()->toArray();

            }
            else
            {
                $bills = BillSubmit::whereRaw($condition)->where('bill_submit.status', '=', 1)->get()->toArray();
           ...
           try{
               ...
               return view('billsubmit::rejectedBill.index', compact('bills', 'branchs', 'branch_currency', 'currency_rate_data'));

           }catch (\Exception $exception){

               $bills = collect($bills);

               return view('billsubmit::rejectedBill.index', compact('bills', 'branchs'));
           }
    ...
    public function search(Request $request)
    {
        ...
        if($branch_id==1){
            ...
            $bills = BillSubmit::whereRaw($condition)->where('bill_submit.status', '=', 1)->select(DB::raw('bill_submit.*'))->get()->toArray();

        }else{

            $bills = BillSubmit::whereRaw($condition)->where('bill_submit.status', '=', 1)->select(DB::raw('bill_submit.*'))->join('users','users.id','=','bill.created_by')->where('branch_id',$branch_id)->get()->toArray();
        ...
        try{
            ...
            return view('billsubmit::rejectedBill.index', compact('bills','branchs','branch_id','from_date','to_date', 'branch_currency', 'currency_rate_data'));
    ...
    public function show($id)
    {
       ...
       return view('billsubmit::rejectedBill.show', compact('customers', 'bill', 'due_bill_sub', 'bill_entry', 'item_category', 'accounts'));
  }
        ...
        try{
            ...
            if($bill->save())
            {
                ...
                if(DB::table('bill_entry')->insert($bill_entry))
                {
                    ...
                    $due_array = array_combine( $due_amount, $due_date);
        ...
        if(isset($request->reject)){
            ...
            if ($dat->update()) {
                ...
                return redirect()->route('bill_submit_rejected_bill')
                                          ->with('alert.status', 'success')
                                          ->with('alert.message', 'Bill has been send to pending');
            }else{
            ...
            else
            {
                ...
                // }else{
                    ...
                    $bill_entry[] = [
                        ...
                        'debit_credit'      => 1,
                        'amount'            => $bill_entry_find[$j]['amount'],
                        'account_name_id'   => $bill_entry_find[$j]['account_id'],
                        'jurnal_type'       => 'bill',
                        'bill_id'           => $bill_id,
                        'contact_id'        => $bill_entry_find[$j]['customer_id'],
                        'created_by'        => $user_id,
